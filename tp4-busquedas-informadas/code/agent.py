...
class Agent:
    ...
    # a* :
    def a_star(self, initial_state, goal_state, size, desc, scenario):
        states = 0
        frontier = queue.PriorityQueue()
        explored = set()
        # Convertimos el goal_state a posición (goal_pos)
        goal_pos = self.state_to_position(goal_state, size)
        frontier.put((0, initial_state, []))
        while not frontier.empty():
            path_cost, current_state, path = frontier.get()
            if current_state == goal_state:  # Llegó al objetivo
                return path, states
            if current_state not in explored:
                explored.add(current_state)
                current_pos = self.state_to_position(current_state, size)
                for action, (ax, ay) in self.ACTIONS.items():
                    ...
                    if self.is_valid_position(next_pos, size):
                        ...
                        if desc[x][y] != 'H':  # Agujero
                            ...
                            new_path_cost = path_cost + step_cost
                            # Manhattan
                            heuristic_cost = abs(next_pos[0] - goal_pos[0]) + abs(next_pos[1] - goal_pos[1])
                            total_cost = new_path_cost + heuristic_cost
                            if next_state not in explored:
                                frontier.put((total_cost, next_state, path + [action]))
                                states += 1
                            else:  # Reemplaza si el costo es menor al que estaba antes
                                for index, (f_cost, f_state, f_path) in enumerate(frontier.queue):
                                    if f_state == next_state and total_cost < f_cost:
                                        frontier.queue[index] = (total_cost, next_state, path + [action])
                                        break
